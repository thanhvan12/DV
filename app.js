...
// Parse ngày linh hoạt + serial Excel
...
function initFromHash(){
  ...
  return Math.min(12, Math.max(1, saved));
}
window.addEventListener("keydown", (e) => {
  const cur = +localStorage.getItem("lastQ") || 1;
  if (e.key === "ArrowRight") loadQ(Math.min(12, cur+1), true);
  if (e.key === "ArrowLeft")  loadQ(Math.max(1, cur-1), true);
});
window.addEventListener("hashchange", () => {
  const i = initFromHash(); if (!isNaN(i)) loadQ(i, false);
});
...
/* =========================
   DISPATCHER
========================= */
async function loadQ(i, pushHash){
  d3.select("#chart").html(""); tip.style("opacity",0).style("display",null);
  setActive(i);
  switch(+i){
    case 1: await drawQ1(); break;
    case 2: await drawQ2(); break;
    case 3: await drawQ3(); break;
    case 4: await drawQ4(); break;
    case 5: await drawQ5(); break;
    case 6: await drawQ6(); break;
    case 7: await drawQ7(); break;
    case 8: await drawQ8(); break;
    case 9: await drawQ9(); break;
    case 10: await drawQ10(); break;
    case 11: await drawQ11(); break;
    case 12: await drawQ12(); break;
  }
}
...
async function drawQ1(){
  ...
  ).sort((a,b)=>d3.descending(a.value,b.value)).slice(0,20);
  ...
  const x = d3.scaleLinear().domain([0, d3.max(agg,d=>d.value)]).nice()
    .range([margin.left, width - margin.right]);
  const y = d3.scaleBand().domain(agg.map(d=>d.item))
    .range([margin.top, height - margin.bottom]).paddingInner(gap / barH);
  const color = d3.scaleOrdinal(d3.schemeTableau10).domain([...new Set(agg.map(d=>d.group))]);
  ...
  svg.append("g").selectAll("rect").data(agg).join("rect")
    ...
    .attr("height", y.bandwidth()).attr("width", d => x(d.value)-x(0))
    .attr("fill", d => color(d.group))
    .on("pointerenter",(ev,d)=>{
      tip.html(
        `<div><b>Mặt hàng:</b> ${d.item}</div>
         <div><b>Nhóm hàng:</b> ${d.group}</div>
         <div><b>Doanh số bán:</b> ${fmtVNDTrieu(d.value)}</div>
         <div class="sub">Số lượng bán: ${fmtInt(d.qty||0)} SKUs</div>`
      ).style("opacity",1);
    })
    .on("pointermove", ev => tip.style("left",(ev.clientX+12)+"px").style("top",(ev.clientY+12)+"px"))
    .on("pointerleave", ()=> tip.style("opacity",0));

  // labels
  svg.append("g").selectAll("text.value").data(agg).join("text")
    .attr("class","value")
    .attr("x", d => Math.min(x(d.value)+6, width - margin.right - 4))
    .attr("y", d => y(d.item)+y.bandwidth()/2).attr("dy",".35em")
  ...
  legend.insert("rect",":first-child").attr("x",bbox.x-pad).attr("y",bbox.y-pad)
...
async function drawQ2(){
  const rows = await loadCSV(CSV_PATH);
  const mapped = rows.map(r => ({
    groupKey: tidy(r["Mã nhóm hàng"]),
    group   : `[${tidy(r["Mã nhóm hàng"])}] ${tidy(r["Tên nhóm hàng"])}`,
    value   : toNumber(r["Thành tiền"]),
    qty     : (Number.isFinite(toIntQty(r["SL"])) ? toIntQty(r["SL"]) : null)
  })).filter(d=>d.groupKey && Number.isFinite(d.value));
  ...
  const barH=34, gap=8, height = margin.top + margin.bottom + data.length*barH;

  const x = d3.scaleLinear().domain([0,d3.max(data,d=>d.value)]).nice()
    .range([margin.left, width - margin.right]);
  const y = d3.scaleBand().domain(data.map(d=>d.group))
    .range([margin.top, height - margin.bottom]).paddingInner(gap/barH);
  const color = d3.scaleOrdinal(d3.schemeTableau10).domain(data.map(d=>d.group));
  ...
  svg.append("g").selectAll("text.value").data(data).join("text")
    .attr("class","value").attr("x",d=>x(d.value)+6)
    .attr("y",d=>y(d.group)+y.bandwidth()/2).attr("dy",".35em")
    .text(d=>fmtVNDTrieu(d.value));
}
...
async function drawQ3(){
  const rows = await loadCSV(CSV_PATH);
  const DATE_COL="Thời gian tạo đơn", VALUE_COL="Thành tiền";
  const mapped = rows.map(r=>{
    const dt = parseDateAny(r[DATE_COL]); const m = dt? +d3.timeFormat("%m")(dt) : null;
    const val = toNumber(r[VALUE_COL]); const qty = toIntQty(r["SL"]);
    return { m, value: val, qty: Number.isFinite(qty)?qty:null };
  }).filter(d=>d.m && Number.isFinite(d.value) && d.value>0);

  const rolled = d3.rollups(mapped, v=>({
    value: d3.sum(v,d=>d.value),
    qty  : v.some(d=>d.qty!=null)? d3.sum(v,d=>d.qty||0): null
  }), d=>d.m);
  const byMonth = new Map(rolled.map(([m,o])=>[m,o]));
  const data = d3.range(1,13).map(m => ({
    m, value: (byMonth.get(m)?.value)||0, qty:(byMonth.get(m)?.qty ?? null)
  }));

  const width = Math.max(1000, document.querySelector("#chart").clientWidth || 1000);
  const height=520, margin={top:60,right:30,bottom:50,left:70};
  ...
  svg.append("g").selectAll("rect").data(data).join("rect")
    ...
    .on("pointerenter",(ev,d)=>{
      tip.html(
        `<div><b>Tháng ${String(d.m).padStart(2,"0")}</b></div>
         <div><b>Doanh số bán:</b> ${fmtVNDTrieu(d.value)}</div>` +
        (d.qty!=null?`<div class="sub">Số lượng bán: ${fmtInt(d.qty)} SKUs</div>`:"")
      ).style("opacity",1);
    })
  ...
  svg.append("g").selectAll("text.val").data(data).join("text")
    .attr("x",(d,i)=>x(labels[i])+x.bandwidth()/2).attr("y",d=>y(d.value)-6)
    .attr("text-anchor","middle").attr("font-size",12).attr("fill","#333")
    .text(d=>d.value>0?fmtVNDTrieu(d.value):"");
}

/* =========================
   Q4 – Doanh số TB theo Ngày trong tuần
========================= */
async function drawQ4(){
  const rows = await loadCSV(CSV_PATH);
  const DATE_COL="Thời gian tạo đơn", VALUE_COL="Thành tiền";
  const fmtDateKey = d3.timeFormat("%Y-%m-%d");
  const daily = new Map(); // date -> {total, qtyTotal, dow}
  const dowMonToSun = d => ((d.getDay()+6)%7)+1;

  rows.forEach(r=>{
    const dt = parseDateAny(r[DATE_COL]); const val = toNumber(r[VALUE_COL]); const qty = toIntQty(r["SL"]);
    if (!dt || !Number.isFinite(val)) return;
    const day = new Date(dt.getFullYear(), dt.getMonth(), dt.getDate());
    const key = fmtDateKey(day); const dow = dowMonToSun(day);
    const cur = daily.get(key);
    if (cur){ cur.total+=val; if(Number.isFinite(qty)) cur.qtyTotal += qty; }
    else { daily.set(key,{total:val, qtyTotal:Number.isFinite(qty)?qty:0, dow}); }
  });

  const sums=Array(8).fill(0), qtySums=Array(8).fill(0), counts=Array(8).fill(0);
  daily.forEach(({total, qtyTotal, dow})=>{
  ...
  const width=Math.max(1000, document.querySelector("#chart").clientWidth||1000);
  const height=520, margin={top:40,right:20,bottom:50,left:70};
  const x=d3.scaleBand().domain(data.map(d=>d.label)).range([margin.left,width-margin.right]).padding(0.25);
  const y=d3.scaleLinear().domain([0,d3.max(data,d=>d.avg)]).nice().range([height-margin.bottom,margin.top]);
  ...
  svg.append("g").selectAll("rect").data(data).join("rect")
    .attr("x",d=>x(d.label)).attr("y",d=>y(d.avg)).attr("width",x.bandwidth()).attr("height",d=>y(0)-y(d.avg))
    .attr("fill",d=>color(d.dow))
    .on("pointerenter",(ev,d)=>{
      tip.html(
        `<div><b>${d.label}</b></div>
         <div><b>Doanh số bán TB:</b> ${fmtVND(d.avg)}</div>` +
...
/* =========================
   Q5 – Doanh số TB theo Ngày trong tháng
========================= */
async function drawQ5(){
  const rows = await loadCSV(CSV_PATH);
  const DATE_COL="Thời gian tạo đơn", VALUE_COL="Thành tiền";
  ...
  daily.forEach((v,key)=>{
    ...
    const o=bucket.get(d); o.sumVal+=v.total; o.sumQty+=v.qtyTotal||0; o.count++;
  });
  ...
  const width=Math.max(1000, document.querySelector("#chart").clientWidth||1000);
  const height=520, margin={top:40,right:20,bottom:50,left:70};
  const x=d3.scaleBand().domain(data.map(d=>d.day)).range([margin.left,width-margin.right]).padding(0.15);
  const y=d3.scaleLinear().domain([0,d3.max(data,d=>d.avg)]).nice().range([height-margin.bottom,margin.top]);
  const palette = d3.schemeTableau10.concat(d3.schemeSet3);
  const color=d3.scaleOrdinal().domain(data.map(d=>d.day)).range(palette.slice(0,31));
  ...
  svg.append("g").selectAll("rect").data(data).join("rect")
    .attr("x",d=>x(d.day)).attr("y",d=>y(d.avg)).attr("width",x.bandwidth()).attr("height",d=>y(0)-y(d.avg))
    .attr("fill",d=>color(d.day))
    .on("pointerenter",(ev,d)=>{
      tip.html(
        `<div><b>Ngày ${String(d.day).padStart(2,"0")}</b></div>
         <div><b>Doanh số bán TB:</b> ${fmtVND(d.avg)}</div>`+
        (d.avgQty!=null?`<div class="sub">Số lượng bán TB: ${fmtInt(d.avgQty)} SKUs</div>`:"")
      ).style("opacity",1);
    })
...
/* =========================
   Q6 – Doanh số TB theo Khung giờ
========================= */
async function drawQ6(){
  const rows = await loadCSV(CSV_PATH);
  const DATE_COL="Thời gian tạo đơn", VALUE_COL="Thành tiền";
  const recs = rows.map(r=>{
    const dt = parseDateAny(r[DATE_COL]); if (!dt) return null;
    const v = toNumber(r[VALUE_COL]); const q = toIntQty(r["SL"]);
    if (!Number.isFinite(v) && !Number.isFinite(q)) return null;
    const dayKey = d3.timeFormat("%Y-%m-%d")(dt);
    return { h: dt.getHours(), d: dayKey, v: Number.isFinite(v)?v:0, q: Number.isFinite(q)?q:0 };
  }).filter(Boolean);
  ...
  const height=520, margin={top:36,right:24,bottom:60,left:70};
  const x=d3.scaleBand().domain(stats.map(d=>d.label)).range([margin.left,width-margin.right]).padding(0.18);
  const y=d3.scaleLinear().domain([0,d3.max(stats,d=>d.avg)]).nice().range([height-margin.bottom,margin.top]);
  const color=d3.scaleSequential(d3.interpolateTurbo).domain([0,23]);

  const svg=d3.select("#chart").append("svg").attr("width",width).attr("height",height);
  svg.append("g").attr("transform",`translate(0,${height-margin.bottom})`).call(d3.axisBottom(x))
    .selectAll("text").attr("font-size",11).attr("transform","translate(0,5)");
  svg.append("g").attr("transform",`translate(${margin.left},0)`)
...
========================= */
async function drawQ7(){
  const rows = await loadCSV(CSV_PATH);
  const ORDER_COL="Mã đơn hàng", GROUP_CODE="Mã nhóm hàng", GROUP_NAME="Tên nhóm hàng";
  const totalOrders = new Set(rows.map(r => tidy(r[ORDER_COL]))).size;

  const groupToOrderSet = new Map();
  rows.forEach(r=>{
    const key = `[${tidy(r[GROUP_CODE])}] ${tidy(r[GROUP_NAME])}`.trim();
    const ord = tidy(r[ORDER_COL]);
    if (!key || !ord) return;
    if (!groupToOrderSet.has(key)) groupToOrderSet.set(key,new Set());
    groupToOrderSet.get(key).add(ord);
  });

  const data = Array.from(groupToOrderSet, ([group, ids]) => ({
  ...
  svg.append("g").selectAll("rect").data(data).join("rect")
    .attr("x",x(0)).attr("y",d=>y(d.group)).attr("width",d=>x(d.p)-x(0)).attr("height",y.bandwidth())
    .attr("fill",d=>color(d.group))
    .on("pointerenter",(ev,d)=>{
      tip.html(
        `<div><b>Nhóm hàng:</b> ${d.group}</div>
         <div><b>SL Đơn Bán:</b> ${fmtInt(d.count)}</div>
         <div><b>Xác suất Bán:</b> ${d3.format(".1%")(d.p)}</div>`
      ).style("opacity",1);
    })
    .on("pointermove", ev=>tip.style("left",(ev.clientX+12)+"px").style("top",(ev.clientY+12)+"px"))
    .on("pointerleave", ()=>tip.style("opacity",0));
  ...
  svg.append("g").selectAll("text.label").data(data).join("text")
    .attr("x",d=>x(d.p)+8).attr("y",d=>y(d.group)+y.bandwidth()/2+4).attr("fill","#333").attr("font-size",12)
...
========================= */
async function drawQ8(){
  const rows = await loadCSV(CSV_PATH);
  const DATE_COL="Thời gian tạo đơn", ORDER_COL="Mã đơn hàng", GROUP_CODE="Mã nhóm hàng", GROUP_NAME="Tên nhóm hàng";

  const recs = rows.map(r=>{
    const dt=parseDateAny(r[DATE_COL]);
    const m=dt? +d3.timeFormat("%m")(dt) : null;
    const order=tidy(r[ORDER_COL]); const code=tidy(r[GROUP_CODE]); const name=tidy(r[GROUP_NAME]);
    return { m, order, group:`[${code}] ${name}`.trim() };
  }).filter(d=>d.m && d.order && d.group);

  const monthToOrderSet = new Map();
  recs.forEach(d => { if(!monthToOrderSet.has(d.m)) monthToOrderSet.set(d.m,new Set()); monthToOrderSet.get(d.m).add(d.order); });

  const gmToOrderSet = new Map(); const groups=new Set(); const gmKey=(g,m)=>`${g}|${m}`;
  recs.forEach(d=>{
    groups.add(d.group);
  ...
  const series = Array.from(groups).sort().map(g=>{
    const values = months.map(m=>{
      ...
      return { m, p, numer, denom, group:g };
    });
    return { group:g, values };
  });

  const width=Math.max(1100, document.querySelector("#chart").clientWidth||1100);
  const height=520, margin={top:40,right:170,bottom:46,left:60};
  const svg=d3.select("#chart").append("svg").attr("width",width).attr("height",height);
  ...
  series.forEach(s=>{
    gLines.append("path").datum(s.values).attr("fill","none").attr("stroke",color(s.group)).attr("stroke-width",2.5).attr("d",line);
    const layer=gLines.append("g").attr("data-group",s.group);
    layer.selectAll("circle").data(s.values).join("circle")
      .attr("cx",d=>x(d.m)).attr("cy",d=>y(d.p)).attr("r",4).attr("fill",color(s.group))
      .on("pointerenter",(ev,d)=>{
        tip.html(
          `<div><b>Tháng ${String(d.m).padStart(2,"0")} | Nhóm hàng ${d.group}</b></div>
           <div class="sub">SL Đơn Bán: ${fmtInt(d.numer)}</div>
           <div class="sub">Xác suất Bán: ${d3.format(".1%")(d.p)}</div>`
        ).style("opacity",1);
      })
      .on("pointermove", ev=>tip.style("left",(ev.clientX+12)+"px").style("top",(ev.clientY+12)+"px"))
      .on("pointerleave", ()=>tip.style("opacity",0));
    ...
    layer.selectAll("text").data(s.values).join("text")
      .attr("x",d=>x(d.m)).attr("y",d=>y(d.p)-6).attr("text-anchor","middle")
      .attr("font-size",11).attr("fill",color(s.group)).text(d=>d3.format(".0%")(d.p));
  });

  const legend=svg.append("g").attr("class","legend").attr("transform",`translate(${width - margin.right + 10}, ${margin.top})`);
  const leg=legend.selectAll("g").data(series.map(s=>s.group)).join("g").attr("transform",(d,i)=>`translate(0,${i*20})`);
  leg.append("rect").attr("width",12).attr("height",12).attr("fill",d=>color(d));
  leg.append("text").attr("x",16).attr("y",10).text(d=>d);
}

/* =========================
   Q9 – Xác suất của Mặt hàng theo Nhóm hàng
========================= */
async function drawQ9(){
  const rows = await loadCSV(CSV_PATH);
  const COL = {
    order:"Mã đơn hàng", gcode:"Mã nhóm hàng", gname:"Tên nhóm hàng",
    itemCode:"Mã mặt hàng", itemName:"Tên mặt hàng", groupBoth:"Mã và tên nhóm hàng"
  };
  const data = rows.map(r=>{
    const order=tidy(r[COL.order]);
    const gkey = (COL.groupBoth in r && tidy(r[COL.groupBoth])) ? tidy(r[COL.groupBoth])
  ...
  const grid=d3.select("#chart").append("div").attr("class","gridwrap");
  ...
  groups.forEach(groupName=>{
    ...
    const outerW=440, W=outerW-margin.left-margin.right, H=Math.max(150, items.length*26);

    const card=grid.append("div").attr("class","subplot");
    card.append("h3").text(groupName);
    if (!denom){ card.append("div").text("Không có đơn hàng trong nhóm này."); return; }
    ...
    g.selectAll("text.value").data(items).join("text")
      .attr("class","value").attr("x",d=>x(d.proba)+6).attr("y",d=>y(d.name)+y.bandwidth()/2).attr("dy",".35em")
...
========================= */
async function drawQ10(){
  const rows=await loadCSV(CSV_PATH);
  const DATE_COL="Thời gian tạo đơn", ORDER_COL="Mã đơn hàng", GCODE_COL="Mã nhóm hàng", GNAME_COL="Tên nhóm hàng", ITCODE_COL="Mã mặt hàng", ITNAME_COL="Tên mặt hàng";

  const data = rows.map(r=>{
    const order=tidy(r[ORDER_COL]), gcode=tidy(r[GCODE_COL]), gname=tidy(r[GNAME_COL]);
    const icode=tidy(r[ITCODE_COL]), iname=tidy(r[ITNAME_COL]);
    const dt=parseDateAny(r[DATE_COL]); const m=dt? "T"+String(dt.getMonth()+1).padStart(2,"0"):null;
  ...
  groups.forEach((arr,g)=>{
    ...
    numMap.forEach((itemMap,m)=>{
      ...
      itemMap.forEach((cnt,item)=>recs.push({ group:g, month:m, item, prob: denom?cnt/denom:0, cnt, denom }));
    });
    ...
    const card=grid.append("div").attr("class","subplot");
    card.append("h3").text(g);
    const svg=card.append("svg").attr("class","chart-area").attr("width",400).attr("height",220);
...
========================= */
async function drawQ11(){
  const rows=await loadCSV(CSV_PATH);
  const CUSTOMER_COL="Mã khách hàng", ORDER_COL="Mã đơn hàng";

  const map=new Map();
  rows.forEach(r=>{
    const kh=tidy(r[CUSTOMER_COL]); const od=tidy(r[ORDER_COL]);
    if(!kh||!od) return; if(!map.has(kh)) map.set(kh,new Set()); map.get(kh).add(od);
  });
  ...
  const width=Math.max(1100, document.querySelector("#chart").clientWidth||1100);
  const height=520, margin={top:50,right:20,bottom:50,left:70};
  const svg=d3.select("#chart").append("svg").attr("width",width).attr("height",height);
  ...
  svg.append("g").attr("transform",`translate(0,${height-margin.bottom})`)
    .call(d3.axisBottom(x)).selectAll("text").attr("font-size",12);
  svg.append("g").attr("transform",`translate(${margin.left},0)`)
    .call(d3.axisLeft(y).ticks(8).tickFormat(d3.format(","))).selectAll("text").attr("font-size",12);

  svg.append("text").attr("x",margin.left-50).attr("y",margin.top-20)
    .attr("font-size",13).attr("font-weight","600").text("Số khách hàng");
}

/* =========================
   Q12 – Phân phối Mức chi trả của Khách hàng
========================= */
async function drawQ12(){
  const rows=await loadCSV(CSV_PATH);
  const CUSTOMER_COL="Mã khách hàng", SALES_COL="Thành tiền", BIN_STEP=50_000;
  function parseMoney(v){ if(v==null||v==="") return 0; const s=String(v).replace(/[^0-9.-]/g,""); const n=+s; return isNaN(n)?0:n; }

  const spendByCustomer=new Map();
  rows.forEach(r=>{
    const kh=tidy(r[CUSTOMER_COL]); if(!kh) return;
  ...
  if(!totals.length){ d3.select("#chart").text("Không có tổng chi tiêu hợp lệ."); return; }
  ...
  const data=bins.map(b=>({ x0:b.x0, x1:b.x1, label:fmtNum(Math.round(b.x0)), SoKhach:b.length }));

  const width=Math.max(1400, document.querySelector("#chart").clientWidth||1200);
  const height=560, margin={top:54,right:20,bottom:80,left:74};
  const svg=d3.select("#chart").append("svg").attr("width",width).attr("height",height);
  ...
  svg.append("g").selectAll("rect").data(data).join("rect")
    .attr("x",d=>x(d.label)).attr("y",d=>y(d.SoKhach)).attr("width",x.bandwidth())
    .attr("height",d=>(height-margin.bottom)-y(d.SoKhach)).attr("fill","steelblue")
    .on("pointerenter",(ev,d)=>{
      tip.html(
        `<div><b>Đã chi tiêu Từ ${fmtNum(d.x0)} đến ${fmtNum(d.x1)}</b></div>
         <div class="sub">Số lượng KH: ${fmtNum(d.SoKhach)}</div>`
      ).style("opacity",1);
    })
    .on("pointermove", ev=>tip.style("left",(ev.clientX+12)+"px").style("top",(ev.clientY+12)+"px"))
    .on("pointerleave", ()=>tip.style("opacity",0));
